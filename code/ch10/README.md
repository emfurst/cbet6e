...
The full list, including the §10.3 notebooks, is in
[`revision_notes/c10.md`](../../revision_notes/c10.md) §2.
...
2. **Both products leave as bottoms**, and which product leaves which port is *computed*, not
   ...
   presuming the staircase's direction. That is exactly what erratum **E17** got wrong in the
   5e, where column 2's distillate and bottoms are swapped.
...
- ⛔ **The ascending staircase was then drawn reflected in the diagonal.** Its corners must lie
  **on the equilibrium curve** — ascending that corner is $(x_n, f(x_n))$, so the step goes *up*
  ...
  curve nowhere. ⚠️ **The stage counts were unaffected**, which is why it passed every numerical
...
- ⭐ **The bottoms line marks where the staircase lands, not the specification** (author,
  ...
  the spec named in the label: `bottoms 0.022 (spec 0.05 met)`. ⚠️ Production must not "correct"
...
- ⚠️ **Column 1's "22 stages" was an artifact of starting at the azeotrope**, which is a fixed
...
(`research/references/ch10/je60035a018.pdf`) — nine points for triethylamine / *n*-hexane
at 60 °C. Not digitized from the 5e art, which [[5e-art-not-metrological]] forbids.
⚠️ **The paper's component 1 is triethylamine and the book's abscissa is hexane**, so
every composition is flipped once, explicitly, on read.

⚠️ **The transcription was checked against the table's own identity**, not by re-reading
...
⚠️ Those imply pure vapor pressures 1.6 % and 0.3 % from the **0.3843 / 0.7583** SIS
...
Printed keys resolve through `cbethermo.org/<key>` to the notebook page. Generated by
`6e_companion_site/tools/figures_qr.py` from `source/chapters/ch10.yaml`; the print
manifest is `build/figures_print_manifest_ch10.md`.
...
⚠️ **None of the fourteen has been pasted into `c10.docx` yet** — the placeholder tracker
reports all of ch10 outstanding, which is a chapter-wide task and not a defect in any one
figure. The markers to paste are in `6e_companion_site/build/qr_placeholder_map.md`.

⚠️ Run the generator **with segno** or it reports success and silently stages the old
art: `cd 6e_companion_site && uv run --with segno --with pyyaml python3 tools/figures_qr.py`.
Run `build.py` first, or the redirects 404.

## Print art

| file | staged as | figure |
|---|---|---|
| `pdf/Illustration_10.1-1a.pdf` | `deliverables/figures/print-bw/ch10/c10uf001a.pdf` | x-y, 50 °C |
...
⛔ **The `c10uf00N` names for in-illustration art are not the names the .docx uses**, and
this is the ch10 form of [[5e-art-filenames-offset]]: the manuscript's `Insert figure …`
slugs restart at `c10f001` inside each illustration, so Illustration 10.2-2's two figures are
slugged `c10f001.pdf`/`c10f002.pdf` and **collide with Figures 10.1-1 and 10.1-2**, and
Figures 10.2-2/10.2-3 are slugged `c10f002.pdf`/`c10f003.pdf`, colliding again. The staged
names above are the correct ones — see [`revision_notes/c10.md`](../../revision_notes/c10.md)
§1 for the whole verified map.

⭐ **Illustration 10.2-4's two captions are recovered.** Both were lost from the .docx —
bare `Figure 1` / `Figure 2` slugs with no text (§1 of the chapter notes listed this as an
open gap). Read off the 5e page: **Figure 1** is Gᴱ and ΔG_mix against x_B; **Figure 2**
is the species partial pressures against x_B with the Raoult's law lines dashed.

⚠️ **Figure 2 departs from the 5e deliberately.** The 5e draws it on twin ordinates at a
...
`get_fonts`), per [[figure-typography-cm]].

⚠️ **Figure d is drawn as a T-x-y and the 5e caption calls it a P-x-y.** The caption is
wrong (E5) and needs fixing in the manuscript; the art is right.
...
[`illustration_10.1-1_aspen_validation.ipynb`](validation/illustration_10.1-1_aspen_validation.ipynb)
checks this chapter's first notebook against **the Aspen Plus output Sandler shipped with
the 5e** — the spreadsheet the illustration's bracketed note points at, recovered from
`legacy-5e/5e_companion_site/files/aspen/ASPEN_for_textbook.zip` (`Chapter 10-1/Illus
10.1-1/Illus 10.1-1.xlsx`, 82 rows) and extracted to
`validation/aspen_illustration_10.1-1.csv`.
...
Excluded from the public repository by the same rule as the other `validation/` folders —
which is also why the Aspen-derived CSV lives here and not in `code/data/`.
...
The full argument is in [`revision_notes/c10.md`](../../revision_notes/c10.md) §7.4.
...
⚠️ **Two of the checks failed, and the book is what was wrong.** Illustration 10.2-2(a)'s
...
**K₅ = 2.7406** contradicts the same page's vapor pressure and should be **2.686**. Both
are filed in [`revision_notes/c10.md`](../../revision_notes/c10.md) §2 as E14 and E15.
E14 is now **superseded** — that illustration is methyl acetate / methanol in the 6e, so
there is no printed number left to correct — but the validation stands: those checks are
...
- ⚠️ **Three correlation forms, not one.** The chapter uses ln P = A − B/(T+C),
  ln P = A − ΔH_vap/RT, and log₁₀ P = −A/T + B. A module offering only "Antoine" cannot
  run the chapter.
- ⚠️ **Do not filter NaNs before plotting.** `pxy`/`txy` return NaN where the model has no
...
- ⛔ **The .ipynb on disk is canonical.** Per [[ch6-figure-notebook-handtuning]], never
  regenerate a notebook from a builder script once it exists — edit it cell by cell.
