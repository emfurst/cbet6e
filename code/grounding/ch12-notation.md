...
- The mixture fugacity and mixture fugacity coefficient. $\bar{f}_i$ is the fugacity of species $i$ IN A MIXTURE and $\bar{\phi}_i = \bar{f}_i/(x_i P)$ its fugacity coefficient. BOTH CARRY AN OVERBAR — the same overbar Ch.8 introduced for partial molar properties, because both are mixture properties of species $i$. Distinguish them from the Ch.6-7 pure-fluid quantities, which carry NO accent: $f$ is the fugacity of a pure fluid and $\phi = f/P$ its fugacity coefficient. So $\phi$ (no accent) is a pure-fluid property and $\bar{\phi}_i$ (overbar, species subscript) is a mixture property. ⚠️ A caret is NEVER used for either; if you have seen $\hat{\phi}_i$ attributed to this book, it is wrong.
...
| `\alpha` | NRTL nonrandomness parameter (⚠️ NOT the van Laar $\alpha$ of Eq. 9.5-9, which is a different quantity in the same chapter) | - |
| `\delta_i` | solubility parameter of species $i$ (Table 9.6-1, regular solution theory). ⚠️ tabulated in calorie-based units; the code converts on read | (J/m^3)^{1/2} |
...
| `z` | lattice coordination number in UNIQUAC/UNIFAC, taken as 10 (⚠️ not a compressibility factor $Z$, and not an ionic charge $z_+$) | - |
...
| `H_i(T,P)` | Henry’s law constant of species $i$, mole-fraction basis (⚠️ collides visually with enthalpy $H$) | kPa |
...
| `M_i` | molality of species $i$ (⚠️ $M$ is molecular weight in Ch.1-3; with a species subscript here it is molality) | mol/kg solvent |
...
- **(9.4-9) Combining rule and the binary interaction parameter** — `a_{ij} = \sqrt{a_i a_j}\,(1 - k_{ij})`
  The geometric mean, corrected by a fitted binary interaction parameter $k_{ij}$. $k_{ij}$ is small (order 0.1 or less) but the results are sensitive to it. ⚠️ A missing $k_{ij}$ is NOT safely assumed zero — say so rather than defaulting silently; the book tabulates values in Table 9.4-1.
- **(9.5-9) van Laar activity coefficients** — `\ln\gamma_1 = \dfrac{\alpha}{\left(1+\dfrac{\alpha x_1}{\beta x_2}\right)^{2}}, \qquad \ln\gamma_2 = \dfrac{\beta}{\left(1+\dfrac{\beta x_2}{\alpha x_1}\right)^{2}}`
  A two-parameter correlative model, derived from the Wohl expansion with $\alpha = 2q_1a_{12}$ and $\beta = 2q_2a_{12}$. ⚠️ This $\alpha$ is NOT the NRTL nonrandomness parameter $\alpha$, which appears four pages later in the same chapter.
...
- **(9.6-13b) Modified UNIFAC group-interaction parameter** — `\Psi_{mn} = \exp\left[-\dfrac{a_{mn}+b_{mn}T+c_{mn}T^{2}}{T}\right]`
  The temperature-dependent three-parameter form, equivalently $\exp[-(u_{mn}-u_{nn})/kT]$ with $(u_{mn}-u_{nn})/k = a_{mn}+b_{mn}T+c_{mn}T^2$. ⛔ $a_{mn}\neq a_{nm}$, so $\Psi_{mn}\neq\Psi_{nm}$ — see convention interaction_parameter_asymmetry. ⚠️ Original UNIFAC uses the one-parameter $\exp(-a_{mn}/T)$; if you are given a single $a_{mn}$ per pair, you have original-UNIFAC parameters and they do not belong with Table 9.5-2’s $R$ and $Q$.
