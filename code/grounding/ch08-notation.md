...
*Scope: through Chapter 8 (The Thermodynamics of Multicomponent Mixtures).* This file documents the symbols, units, and sign conventions
...
- Two "in a mixture" quantities are written with DIFFERENT accents. The fugacity of species $i$ in a mixture uses an OVERBAR, $\bar{f}_i$ (like a partial molar quantity). The fugacity coefficient of species $i$ in a mixture uses a CARET, $\hat{\phi}_i = \bar{f}_i / P$. Here the caret does NOT mean "per unit mass" (its Ch.1-7 meaning) — it marks a mixture-component property. Do not read $\hat{\phi}_i$ as a specific/per-mass quantity, and do not confuse the pure-fluid $f$, $\phi$ of Ch.6-7 with these mixture-component versions. (Both symbols appear in the Ch.8 notation summary but are developed in Ch.9.)
...
| `\bar{f}_i` | fugacity of species $i$ in a mixture (mixture generalization of the pure-fluid $f$; developed in Ch.9) | kPa |
| `\hat{\phi}_i` | fugacity coefficient of species $i$ in a mixture ($= \bar{f}_i/P$; caret does NOT mean per-mass) | - |
