...
*Scope: through Chapter 15 (Some Additional Biochemical Applications of Thermodynamics).* This file documents the symbols, units, and sign conventions
...
- The CARET has two readings, and which one applies depends on the symbol it is drawn over. Over a THERMODYNAMIC PROPERTY it means PER UNIT MASS — $\hat{V}$ is specific volume, $\hat{H}$ specific enthalpy (Ch.1). Over the MOLAR EXTENT OF REACTION it means PER UNIT VOLUME — $\hat{X}_j = X_j/V$, in mol/m$^3$, defined in Sec. 8.3 and used to write Eq. 8.3-7, where $d\hat{X}_j/dt$ is the rate of reaction per unit volume. Both readings appear in this chapter: $\hat{H}$, $\hat{S}$, $\hat{U}$ and $\hat{V}$ take the first and $\hat{X}_j$ the second. Ch.13 and Ch.14 use $\hat{X}$ on the same rule.
...
- The temperature-independent parameter in Wilson, NRTL and UNIQUAC is the ENERGY, not the quantity these models are usually tabulated with. The book prints all three laws: Wilson $\Lambda_{ij} = (\underline{V}_j/\underline{V}_i)\exp[-(\lambda_{ij}-\lambda_{ii})/RT]$ (Eq. 9.5-11), NRTL $\tau_{ij} = (g_{ij}-g_{jj})/RT$ (Eq. 9.5-13), and UNIQUAC $\ln\tau_{ij} = -(u_{ij}-u_{jj})/RT$. The constants held fixed are the energy differences $\lambda_{ij}-\lambda_{ii}$, $g_{ij}-g_{jj}$ and $u_{ij}-u_{jj}$; $\Lambda_{ij}$ and $\tau_{ij}$ each carry a $1/T$ and THEREFORE VARY WITH TEMPERATURE. A published table of $\tau_{ij}$ is a table for one temperature. Reusing those numbers at a second temperature holds $\tau$ fixed where the model requires it to move, and the result is a plausible number carrying no sign of the error. Van Laar $\alpha$ and $\beta$ carry the same $1/T$. Recover the energy first, then rebuild the model at the temperature required. Redlich-Kister and regular solution are the exceptions: their constants are energies in J/mol and are divided by $RT$ where they are used.
- The two UNIFAC parameter sets NUMBER THEIR SUBGROUPS DIFFERENTLY, and every number in one table also exists in the other, so a group assignment carried across raises no error. Original UNIFAC (Fredenslund, Jones and Prausnitz, 1975) has 85 subgroups in 44 main groups; the temperature-dependent (Dortmund) revision, which is the default in this book and the set printed in Table 9.5-2, has 92 in 47. Where the numbering diverges it diverges completely: subgroups 78-80 are $\mathrm{SiH_3}$, $\mathrm{SiH_2}$ and $\mathrm{SiH}$ in the original set against cy-$\mathrm{CH_2}$, cy-CH and cy-C in Dortmund; 81 and 82 are Si and $\mathrm{SiH_2O}$ against OH(s) and OH(t); 27 is $\mathrm{FCH_2O}$ against cy-$\mathrm{CH_2OCH_2}$; 85 is NMP against $\mathrm{CNH_2}$. Dortmund also SPLITS THE HYDROXYL GROUP by substitution, so subgroup 14 is a primary OH there and a secondary or tertiary alcohol takes 81 or 82, while the original set assigns one OH at 14 to all three. The $R$ and $Q$ surface parameters and the $a_{mn}$ interaction matrix must come from the SAME set as the group assignment. Decompose each molecule with the table belonging to the model in use, and never reuse a decomposition across the two sets.
...
- In this chapter $V$ and $L$ are the number of moles of a mixture that are VAPOR and LIQUID -- or, for a flowing system, the vapor and liquid streams of a column or flash unit. $V$ is therefore NOT the volume of Ch.1-7. The molar volume keeps its underbar and its old meaning, $\underline{V}$, so the two are distinguished by the accent and by nothing else when the symbol stands alone. $B$, $D$ and $F$ are the bottoms, distillate and feed flow rates of a distillation column or flash unit. Ch.7's vapor fraction $\omega^{V}$ is $V/(V+L)$ in this notation.
- A superscript $\mathrm{L}$ or $\mathrm{V}$ on any property names the phase that property belongs to: $\theta^{\mathrm{L}}$ is property $\theta$ in the liquid and $\theta^{\mathrm{V}}$ in the vapor, for any $\theta$ (so $\underline{V}^{\mathrm{L}}$ is the molar volume of the liquid). The letters are the same $L$ and $V$ that stand alone for the moles of each phase: as a superscript the letter is a label, standing alone it is an amount.
- $\underline{x}$ and $\underline{y}$ are the SETS of mole fractions, $\{x_1, x_2, \dots\}$ in the liquid and $\{y_1, y_2, \dots\}$ in the vapor. This is the underbar's COMPOSITION reading. Under a QUANTITY the same mark means "per mole", so $\underline{\theta}$ is a molar property (Ch.1, and the convention underbar_two_readings in Ch.2). The underbar is what separates the whole set $\underline{x}$ from the single mole fraction $x_i$, and a function written as $\gamma_i(T, P, \underline{x})$ depends on the whole composition, not on one component. $\underline{x}$ is introduced in Ch.2; $\underline{y}$ is new here.
- A superscript $\infty$ marks the infinite-dilution limit: $\gamma_i^{\infty} = \lim_{x_i \to 0}\gamma_i$, the activity coefficient of species $i$ when it is the dilute component. Distinguish it from Ch.9's $\gamma_i^{*}$ and $\gamma_i^{\square}$, which are the SAME limit used to define a different standard state (Henry's law, on a mole-fraction and a molality basis); $\gamma_i^{\infty}$ is a value of the Ch.9 $\gamma_i$, not a new activity coefficient.
...
| `a_i` | activity of species $i$, $a_i = x_i\gamma_i$; NOT the equation-of-state attractive parameter $a$ of Ch.6, which carries no species subscript for a pure fluid | - |
| `K_i` | separation or K factor for species $i$, $K_i = y_i/x_i$ -- the single number a flash or a column stage is computed from | - |
| `V` | number of moles of a mixture that are VAPOR, or the vapor stream in a distillation column or flash unit. NOT the volume of Ch.1-7 -- that is $V$ (total) and $\underline{V}$ (molar), and both still appear in Sec. 10.3 | mol (mol/s as a stream) |
| `L` | number of moles of a mixture that are LIQUID, or the liquid stream in a distillation column or flash unit | mol (mol/s as a stream) |
| `B` | flow rate of the bottoms product from a distillation column. NOT the dimensionless equation-of-state parameter $B = bP/RT$ of Ch.6, which Sec. 10.3 uses as well, and not the second virial coefficient $B(T)$ | mol/s |
| `D` | flow rate of the distillate (overheads product) from a distillation column | mol/s |
| `F` | flow rate of the feed to a distillation column or flash unit; an italic $F$, where the script $\mathcal{F}$ of Ch.7 is the number of degrees of freedom | mol/s |
| `z_{i,F}` | mole fraction of species $i$ in the FEED, before the feed splits into a vapor and a liquid; the $F$ subscript is the feed stream $F$ | - |
| `P_i` | partial pressure of species $i$, $P_i = y_iP$. Printed in Ch.9's notation section as well, and omitted from ch09.yaml -- if it is added there, drop this entry | kPa |
| `\underline{y}` | the SET of vapor-phase mole fractions, $\{y_1, y_2, \dots\}$; an underbar that does not mean "per mole" (see the convention). The liquid-side counterpart $\underline{x}$ is introduced in Ch.2 | - |
| `\gamma_i^{\infty}` | activity coefficient of species $i$ at infinite dilution, $\lim_{x_i \to 0}\gamma_i$; the limit the van Laar and Wilson constants are usually reported as | - |
...
- `ch12/solid_solubility_naphthalene_hexane_example.ipynb` — How much solid dissolves, and how much the activity coefficient changes the answer (naphthalene / n-hexane)
- `ch12/supercritical_solubility_naphthalene_co2_figure.ipynb` — Naphthalene in supercritical carbon dioxide, three ways (Figure 12.1-1)
- `ch12/heat_of_fusion_from_solubility_insulin_example.ipynb` — A heat of fusion you cannot measure in a calorimeter (insulin hexamer)
- `ch12/freezing_point_and_eutectic_figure.ipynb` — Freezing points, and the eutectic as a crossing (Figure 12.3-1, ethyl benzene / toluene)
- `ch12/environmental_partitioning_benzo_a_pyrene_example.ipynb` — One chemical, one measurement, five compartments (benzo[a]pyrene)
- `ch12/pcb_bioconcentration_example.ipynb` — Bioconcentration, with a field measurement to check it against (PCBs in St. Lawrence eels)
- `ch13/nitrogen_tetroxide_dissociation_example.ipynb` — Nitrogen tetroxide, three questions about one reaction (Figures 13.1-3 and 13.1-4)
- `ch13/high_pressure_ammonia_equilibrium_example.ipynb` — Ammonia synthesis at high pressure (Figures 13.1-5 and 13.1-6)
- `ch13/acetic_acid_ionization_constant_example.ipynb` — Reaction in the liquid phase: an esterification and an ionization (Figure 13.1-7)
- `ch13/ideal_gas_pressure_and_equilibrium_ratios_example.ipynb` — Pressure, dilution, and the three equilibrium ratios (Figure 13.1-2)
- `ch13/measured_equilibrium_constants_example.ipynb` — When the equilibrium constant is measured, not computed (Figure 13.1-1)
- `ch13/heterogeneous_gas_solid_equilibrium_example.ipynb` — When a solid is one of the reactants: four heterogeneous equilibria (Figure 13.2-1)
- `ch13/solubility_product_silver_chloride_example.ipynb` — The solubility product of silver chloride, and what an inert salt does to it (Figure 13.2-2)
- `ch13/steam_carbon_multireaction_example.ipynb` — Coal and steam: three reactions at once (Figures 13.3-1 and 13.3-2)
- `ch13/coupled_reactions_common_ion_and_atp_example.ipynb` — Two reactions at once, twice over: a common ion and a coupled hydrolysis
- `ch13/combined_chemical_phase_ammonia_example.ipynb` — Reaction and condensation at once: ammonia over water
- `ch13/titration_strong_and_weak_acids_example.ipynb` — Two titration curves, and why all the chemistry is on the left half
- `ch13/deprotonation_benzoyl_tyrosine_figure.ipynb` — When does an acid group carry its charge? (Figure 13.5-1)
- `ch13/dibasic_acid_phthalic_figure.ipynb` — A molecule with two places to lose a proton (Figures 13.6-1 and 13.6-2)
- `ch13/amino_acid_charge_glycine_example.ipynb` — The charge on an amino acid, and how it moves with temperature (Figures 13.6-5, 13.6-6 and 13.6-11)
- `ch13/amino_acid_titration_glycine_example.ipynb` — An amino acid as its own buffer (Figures 13.6-8 and 13.6-9)
- `ch13/protein_charge_lysozyme_figure.ipynb` — Thirty-two ionizable groups on one molecule (Figure 13.6-7)
- `ch14/tank_batch_and_tubular_reactor_ethyl_acetate_example.ipynb` — One reaction, three reactors, and the same heat load (Illustrations 14.1-1, 14.1-2, 14.2-1 and 14.3-1)
- `ch14/adiabatic_reaction_temperature_ethylbenzene_example.ipynb` — The adiabatic reaction temperature, and a disagreement the book prints (Illustration 14.3-2)
- `ch14/maximum_useful_work_availability_example.ipynb` — The most work a fuel can do: methane, gasoline, and a spoonful of sugar (Illustrations 14.5-1, 14.5-2 and 14.5-3)
- `ch14/electrochemical_cell_potentials_example.ipynb` — Cell potentials, equilibrium constants, and pH (Table 14.6-1 and Illustrations 14.6-1, 14.6-2 and 14.6-3)
- `ch15/solubility_vs_pH_examples.ipynb` — Four ionizable solids, one equation, and where each curve breaks (Illustrations 15.1-1, 15.1-2, 15.1-3 and 15.4-2)
- `ch15/protein_solubility_salt_and_temperature_examples.ipynb` — Salting in, salting out, and a precipitation temperature that is really a glass transition (Figures 15.2-1 and 15.2-3)
- `ch15/ligand_binding_cooperative_vs_single_site_example.ipynb` — One binding site or four: why cooperativity is what made large animals possible (Figures 15.3-1 and 15.3-2, Illustrations 15.3-1 and 15.4-1)
- `ch15/protein_denaturation_examples.ipynb` — Proteins unfold when they are cold, too: six figures from one stability curve (Illustrations 15.5-1 and 15.5-2)
- `ch15/bioreactor_yield_factors_examples.ipynb` — Balancing a reactor whose products have no molecular formula (Illustrations 15.7-1 to 15.7-5 and Tables 15.7-1 and 15.7-2)
- `ch15/bioreactor_energy_and_second_law_examples.ipynb` — The second law decides how much a fermenter can make (Illustrations 15.7-6 to 15.7-11)
- `ch15/gibbs_donnan_and_ultracentrifuge_examples.ipynb` — A protein that cannot cross a membrane rearranges everything that can (Illustrations 15.8-1 to 15.8-5 and 15.9-1)
